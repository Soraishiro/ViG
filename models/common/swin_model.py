...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size=7, mlp_ratio=4.0, qkv_bias=True, qk_scale=None, drop=0.0, attn_drop=0.0, drop_path=0.0, norm_layer=nn.LayerNorm, downsample=None, last=False, use_checkpoint=False):
        super().__init__()
        self.window_size = window_size
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], window_size=7, mlp_ratio=4.0, qkv_bias=True, qk_scale=None, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.2, norm_layer=nn.LayerNorm, ape=False, patch_norm=True, out_indices=[1, 2, 3], frozen_stages=-1, use_checkpoint=False, pos_dim=768):
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer), depth=depths[i_layer], num_heads=num_heads[i_layer], window_size=window_size, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])], norm_layer=norm_layer, downsample=partial(PatchMerging, pos_dim=pos_dim) if i_layer < self.num_layers else None, last=None if i_layer < self.num_layers - 1 else True, use_checkpoint=use_checkpoint)
...
def swin_base_win7_384(pretrained=None, pos_dim=1024, **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size=[384, 384], embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], window_size=12, drop_path_rate=0.3, pos_dim=pos_dim, **kwargs)
    ...
    if pretrained is not None:
        if pretrained == 'imagenet':
            ...
            if not os.path.exists(path):
                torch.hub._download_url_to_file(url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22k.pth', dst=path)
