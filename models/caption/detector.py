...
class Detector(nn.Module):
    ...
    def __init__(self, backbone, det_module=None, use_gri_feat=True, use_reg_feat=True, hidden_dim=256):
        super().__init__()
        ...
        if self.use_reg_feat:
            ...
            self.input_proj = nn.ModuleList([nn.Sequential(nn.Conv2d(backbone.num_channels[i], hidden_dim, kernel_size=1), nn.GroupNorm(32, hidden_dim)) for i in range(len(backbone.num_channels))])
    ...
    def forward(self, images: NestedTensor):
        ...
        masks = [F.interpolate(mask[None].float(), size=f.shape[-2:]).to(torch.bool)[0] for l, f in enumerate(features)]
        outputs = {}
        ...
        if self.use_reg_feat:
            ...
            hs, _, inter_ref = self.det_module(srcs, masks)
            outputs['reg_feat'] = hs[-1]
            outputs['reg_mask'] = hs[-1].data.new_full((hs[-1].shape[0], 1, 1, hs[-1].shape[1]), 0).bool()
            outputs['reg_boxes'] = inter_ref[-1]
        return outputs

def build_detector(config):
    pos_dim = getattr(config.model.detector, 'pos_dim', None)
    backbone, _ = swin_base_win7_384(frozen_stages=config.model.frozen_stages, pos_dim=pos_dim)
    det_cfg = config.model.detector
    det_module = build_det_module_with_config(det_cfg) if config.model.use_reg_feat else None
    detector = Detector(backbone, det_module=det_module, hidden_dim=config.model.d_model, use_gri_feat=config.model.use_gri_feat, use_reg_feat=config.model.use_reg_feat)
    if os.path.exists(config.model.detector.checkpoint):
        ...
        print(f'[WARN] Detector checkpoint NOT FOUND: {config.model.detector.checkpoint} — training detector from random initialization. Nếu đây là stage 2+ hoặc freeze_detector=true, kết quả sẽ bị ảnh hưởng nghiêm trọng.')
    return detector
