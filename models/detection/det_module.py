...
class DetectionModule(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_decoder_layers=6, dim_feedforward=1024, dropout=0.1, activation='relu', return_intermediate_dec=True, num_feature_levels=4, dec_n_points=4, drop_path=0.0, num_classes=81, aux_loss=True, with_box_refine=True, num_queries=100):
        super().__init__()
    ...
    def prepare_od_inputs(self, srcs, masks):
        ...
        query_pos, query_tgt = torch.split(query_embed, query_embed.shape[1] // 2, dim=1)
        query_pos = query_pos.unsqueeze(0).expand(srcs[0].shape[0], -1, -1)
        query_tgt = query_tgt.unsqueeze(0).expand(srcs[0].shape[0], -1, -1)
        src_flatten = []
        mask_flatten = []
        spatial_shapes = []
        ...
        return {'tgt': query_tgt, 'src': src_flatten, 'src_spatial_shapes': spatial_shapes, 'src_level_start_index': level_start_index, 'src_valid_ratios': valid_ratios, 'src_padding_mask': mask_flatten, 'query_pos': query_pos, 'reference_points': reference_points}
    ...
    def forward(self, srcs, masks):
        ...
        for lid, layer in enumerate(self.decoder_layers):
            ...
            od_inputs['reference_points'] = self.bbox_refine(bbox_embed, od_inputs['tgt'], od_inputs['reference_points'])
            if self.return_intermediate:
                intermediate.append(od_inputs['tgt'])
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios, src_padding_mask=None):
        if reference_points.shape[-1] == 4:
            reference_points = reference_points[:, :, None] * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None]
        else:
            assert reference_points.shape[-1] == 2
        ...
        tgt = self.norm2(tgt)
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points, src, src_spatial_shapes, src_level_start_index, src_padding_mask)
...
def build_det_module_with_config(cfg):
    return DetectionModule(d_model=cfg.d_model, nhead=cfg.num_heads, num_decoder_layers=cfg.num_layers, dim_feedforward=cfg.dim_feedforward, dropout=cfg.dropout, activation=cfg.activation, num_classes=cfg.num_classes, num_feature_levels=cfg.num_levels, dec_n_points=cfg.num_points, num_queries=cfg.num_queries, return_intermediate_dec=cfg.return_intermediate, aux_loss=getattr(cfg, 'aux_loss', False), with_box_refine=cfg.with_box_refine)
