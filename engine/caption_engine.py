...
def gather_result(value):
    ...
    if isinstance(value, torch.Tensor):
        torch.distributed.all_reduce(value, async_op=False)
        value.mul_(1.0 / torch.distributed.get_world_size())
    return value
...
def evaluate_metrics(model, optimizers, dataloader, text_field, epoch=0, split='test', config=None, train_res=None, writer=None, best_cider=None, which='ft_xe', scheduler=None, log_and_save=True):
    model.eval()
    gen, gts = ({}, {})
    counter = 0
    times = []
    with tqdm(desc=f'Epoch {epoch} - evaluation on {split}', unit='it', total=len(dataloader)) as pbar:
        ...
        for it, batch in enumerate(iter(dataloader)):
            ...
            if it % 100 == 0:
                print(f'Number of iterations: {counter}, batch_size={bs}, Total time per 1 batch: {sum(times) / counter:0.5f}s')
            caps_gen = text_field.decode(out, join_words=False)
            for i, (gts_i, gen_i) in enumerate(zip(batch['captions'], caps_gen)):
                gen_i = ' '.join([k for k, g in itertools.groupby(gen_i)])
                gen[f'{it}_{i}'] = [gen_i]
                gts[f'{it}_{i}'] = gts_i
                res = {'image_id': batch['image_ids'][i], 'caption': gen_i}
                results.append(res)
            pbar.update()
    ...
    gts = _metrics.PTBTokenizer.tokenize(gts)
    gen = _metrics.PTBTokenizer.tokenize(gen)
    scores, _ = _metrics.compute_scores(gts, gen)
    ...
    if log_and_save:
        ...
        if scores['CIDEr'] >= best_cider:
            ...
            save_checkpoint(model, optimizers=optimizers, epoch=epoch, scores=scores, best_ciders=best_ciders, config=config, filename=f'checkpoint_best_{split}.pth', scheduler=scheduler)
            best_cider = scores['CIDEr']
        return best_cider
    else:
        return scores

def inference_coco_test(model, dataloader, text_field, epoch=0, split='test', config=None):
    model.eval()
    gen, gts = ({}, {})
    counter = 0
    times = []
...
def train_xe(model, dataloaders, optimizers, text_field, epoch, rank=0, config=None, scheduler=None, writer=None):
    model.train()
    ...
    with tqdm(desc=f'Epoch {epoch} - train', unit='it', total=len(dataloaders['train'])) as pbar:
        for it, batch in enumerate(dataloaders['train']):
            ...
            if rank == 0:
                writer.add_scalar('backbone_lr', optimizers['backbone'].param_groups[0]['lr'], epoch * len(dataloaders['train']) + it)
                writer.add_scalar('model_lr', optimizers['model'].param_groups[0]['lr'], epoch * len(dataloaders['train']) + it)
                lr = optimizers['model'].param_groups[0]['lr']
    val_loss = evaluate_loss(model, dataloaders['valid'], loss_fn, text_field, epoch, writer)
    if rank == 0:
        save_checkpoint(model=model, optimizers=optimizers, epoch=epoch, scores=[], best_ciders=(0, 0), config=config, filename='checkpoint_last.pth', scheduler=scheduler)
    torch.distributed.barrier()
    return {'loss': running_loss / len(dataloaders['train']), 'reward': 0, 'reward_baseline': 0, 'val_loss': val_loss}

def train_sc(model, dataloaders, optimizers, cider, text_field, tokenizer_pool, device, epoch, config, rank=0, writer=None):
    running_reward = 0.0
    ...
    with tqdm(desc='Epoch %d - train' % epoch, unit='it', total=len(dataloaders['train_dict'])) as pbar:
        for it, batch in enumerate(dataloaders['train_dict']):
            ...
            caps_gt = list(itertools.chain(*([c] * beam_size for c in batch['captions'])))
            ...
            pbar.update()
            if rank == 0:
                writer.add_scalar('backbone_lr', optimizers['backbone'].param_groups[0]['lr'], epoch * len(dataloaders['train_dict']) + it)
                writer.add_scalar('model_lr', optimizers['model'].param_groups[0]['lr'], epoch * len(dataloaders['train_dict']) + it)
    loss_fn = NLLLoss(ignore_index=text_field.vocab.stoi['<pad>'])
    val_loss = evaluate_loss(model, dataloaders['valid'], loss_fn, text_field, epoch, writer)
    ...
    if rank == 0:
        save_checkpoint(model=model, optimizers=optimizers, epoch=epoch, scores=[], best_ciders=(0, 0), config=config, filename='checkpoint_last.pth', scheduler=None)
    torch.distributed.barrier()
