...
class CheckpointHook(HookBase):
    ...
    def __init__(self, save_every_epochs=-1, save_every_iters=-1, save_topk=0, metric=None, which_epochs=[], save_dir='', **kwargs):
        super().__init__()
    ...
    def get_checkpoint(self):
        ...
        checkpoint = {'epoch': self.solver.epoch, 'model': model.state_dict(), 'optimizer': self.solver.optimizer.state_dict(), 'scheduler': lr_scheduler_dict, 'epoch_res': self.solver.epoch_res}
    ...
    def after_step(self):
        if self.save_every_iters != -1 and self.solver.step % self.save_every_iters == 0:
            ...
            save_file = osp.join(self.save_dir, 'checkpoint_{:02d}_iter-{:05d}.pth'.format(self.solver.epoch, self.solver.step))
...
class TensorboardHook(HookBase):
    ...
    def after_step(self):
        if self.solver.step % self.log_every_step == 0:
            ...
            self.writer.add_scalars(main_tag=self.name + '/batch', tag_scalar_dict=log_dict, global_step=self.solver.step + self.solver.epoch * self.steps_per_epoch)
...
class WarmUpLRSchedulerHook(HookBase):
    ...
    def __init__(self, warmup_factor=1.0 / 1000, warmup_iters=None):
        super().__init__()
...
def get_default_train_hooks(args, rank=0):
    train_hooks = [ProgressHook(name='train'), LRSchedulerHook()]
    if rank == 0:
        train_hooks += [CheckpointHook(save_period=1, save_dir=args.save_dir), TensorboardHook(name='train', save_dir=args.save_dir)]
    return train_hooks
