...
class CosineLRScheduler(object):
    ...
    def __init__(self, optimizer, num_epochs, num_its_per_epoch, init_lr=0.0005, min_lr=0.0001, warmup_init_lr=1e-05, warmup_factor=0.1, warmup_epochs=1, **kwargs):
    ...
    def cosine_step(self):
        ...
        return (self.init_lr - self.min_lr) * (1 + math.cos(math.pi * self.global_steps / total_iters)) / 2 + self.min_lr

    def state_dict(self):
        return {'init_lr': self.init_lr, 'warmup_init_lr': self.warmup_init_lr, 'min_lr': self.min_lr, 'num_epochs': self.num_epochs, 'num_its_per_epoch': self.num_its_per_epoch, 'warmup_factor': self.warmup_factor, 'warmup_epochs': self.warmup_epochs, 'global_steps': self.global_steps}
...
def test_lr_scheduler():
    ...
    scheduler = CosineLRScheduler(optimizer, num_epochs, num_its_per_epoch=num_its_per_epoch, init_lr=init_lr, min_lr=min_lr, warmup_init_lr=warmup_init_lr)
    lr = []
    global_steps = 0
