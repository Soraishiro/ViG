...
from utils.vig_params import is_vig_param
from engine.caption_engine import build_optimizers, gather_result
from vicap_dataset import CAPTION_FIELD, RAW_CAPTION_FIELD, TOKENIZER_BACKEND, collect_dataset_contract, ensure_train_vocab_source, get_dataloaders, get_datasets, resolve_ktvic_paths
...
def set_trainability(model, freeze_detector=False, freeze_backbone=False, freeze_grit=False):
    ...
    if freeze_grit:
        ...
        for name, parameter in model_without_ddp.named_parameters():
            if not is_vig_param(name) and any((marker in name for marker in GRIT_STACK_MARKERS)):
...
def load_training_state_from_resume(model, optimizers, scheduler, resume_checkpoint_path):
    ...
    if optimizers.get('vig'):
        if checkpoint.get('optim_vig') is None:
            raise RuntimeError('Resume checkpoint is missing optim_vig for an active ViG optimizer.')
        optimizers['vig'].load_state_dict(checkpoint['optim_vig'])
...
class ExperimentTracker:
    ...
    def __init__(self, config: DictConfig, rank: int):
        ...
        self.run_name = resolve_tracking_run_name(base_name=str(getattr(config.tracking, 'run_name', 'vig-run')), run_dir=str(config.exp.run_dir), use_run_dir_name_suffix=getattr(config.tracking, 'use_run_dir_name_suffix', True), explicit_suffix=getattr(config.tracking, 'run_name_suffix', ''))
...
def train_xe(model, dataloaders, optimizers, epoch, total_epochs=1, trace_every_n_steps=0, step_metrics_every_n_steps=0, observer=None, tracker=None, live_metric_recorder=None, scheduler=None, enable_nvidia_smi_metrics=False, log_gradient_norm=False, max_train_batches=0, config=None):
    ...
    loss_vig_cfg = getattr(config, 'loss_vig', None) if config is not None else None
    if loss_vig_cfg is not None:
        lambda_loc = float(getattr(loss_vig_cfg, 'lambda_loc', 0.0))
        lambda_route = float(getattr(loss_vig_cfg, 'lambda_route', 0.1))
        lambda_div = float(getattr(loss_vig_cfg, 'lambda_div', 0.01))
        tau = float(getattr(loss_vig_cfg, 'tau', 0.07))
        warmup_epochs = int(getattr(loss_vig_cfg, 'warmup_epochs', 2))
        ramp_epochs = int(getattr(loss_vig_cfg, 'ramp_epochs', 3))
        loss_eps = float(getattr(loss_vig_cfg, 'eps', 1e-08))
    else:
        lambda_loc = 0.0
        lambda_route = 0.0
        lambda_div = 0.0
        ...
        ramp_epochs = 3
        loss_eps = 1e-08
    model_vig_cfg = getattr(config, 'model_vig', None) if config is not None else None
    lcqm_cfg = getattr(model_vig_cfg, 'lcqm', None) if model_vig_cfg is not None else None
    ...
    if lcqm_enabled and lambda_loc > 0:
        ...
        if missing:
            raise FileNotFoundError(f'[LCQM] FATAL: lambda_loc={lambda_loc} > 0 nhưng thiếu phrase data files:\n  missing: {missing}\n  Cần chạy:\n    1. python vig/tools/mine_local_phrases.py --train_annotations <path/to/train.json>\n    2. python vig/tools/build_phrase_supervision.py --vocab_file data/local_phrase_vocab_v1.json --train_annotations <path/to/train.json>\n  Sau đó resubmit job.')
        import numpy as np
        ...
        for it, batch in enumerate(dataloaders['train']):
            ...
            if should_trace or should_log_step:
                ...
                if alpha_rel is not None:
                    step_payload['vig_alpha_rel'] = round(alpha_rel, 6)
    ...
    if _gate_batches > 0:
        ...
        if epoch >= 3 and gate_mean < 0.05:
            print(f'[GATE] WARN: α4 vẫn < 0.05 sau epoch {epoch + 1} — gate chưa mở. Cân nhắc: tăng vig_lr, giảm gate_bias_init, hoặc bật heuristic bias.')
...
def _lcqm_smoke_forward(config):
    lcqm_enabled = hasattr(config, 'model_vig') and hasattr(config.model_vig, 'lcqm') and getattr(config.model_vig.lcqm, 'enabled', False)
    if not lcqm_enabled:
    ...
    try:
        ...
        lcqm_cfg = SimpleNamespace(d_model=getattr(config.model, 'd_model', 512), **{k: v for k, v in config.model_vig.lcqm.__dict__.items() if not k.startswith('_')})
        ...
        return {'enabled': True, 'passed': not torch.isnan(C_loc).any(), 'c_loc_shape': list(C_loc.shape), 'c_loc_mean_norm': round(float(C_loc.norm(dim=-1).mean()), 4), 'k_loc': getattr(config.model_vig.lcqm, 'k_loc', 16)}
...
def main(gpu, config):
    ...
    try:
        ...
        if phase_plan == 'xe_only':
            ...
            raise RuntimeError(f'Unsupported phase_plan for train_vicap: {phase_plan}')
...
if __name__ == '__main__':
    ...
    try:
        ...
        _cfg = OmegaConf.load('configs/caption/vicap_config.yaml')
