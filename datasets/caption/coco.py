...
class COCO(CPairedDataset):
    ...
    def __init__(self, image_field, text_field, img_root, ann_root, use_restval=True, cut_validation=False, overfit=False, **kwargs):
        self.image_field = image_field
        self.text_field = text_field
        self.overfit = overfit
        roots = {}
        roots['train'] = {'img': os.path.join(img_root, 'train2014'), 'cap': os.path.join(ann_root, 'captions_train2014.json')}
        roots['valid'] = {'img': os.path.join(img_root, 'val2014'), 'cap': os.path.join(ann_root, 'captions_val2014.json')}
        roots['test'] = {'img': os.path.join(img_root, 'val2014'), 'cap': os.path.join(ann_root, 'captions_val2014.json')}
        roots['trainrestval'] = {'img': (roots['train']['img'], roots['valid']['img']), 'cap': (roots['train']['cap'], roots['valid']['cap'])}
    ...
    def split_examples(self):
        return {'train': self.train_examples, 'valid': self.valid_examples, 'test': self.test_examples}
    ...
    def get_samples(self, roots, ids_dataset=None):
        ...
        for split in splits:
            ...
            for index in tqdm(range(len(ids))):
                ...
                widths.append(width)
                example = Example.fromdict({'image_id': img_id, 'image': os.path.join(img_root, filename), 'text': caption, 'tokens': [self.text_field.vocab.stoi[w] for w in self.text_field.preprocess(caption)]})
                if split == 'train':
...
def build_coco_dataloaders(config=None, mode='freezing', device='cpu'):
    ...
    datasets = {'train': CPairedDataset(examples['train'], train_field, overfit=overfit), 'valid': CPairedDataset(examples['valid'], valid_field, overfit=overfit), 'train_dict': CDictionaryDataset(examples['train'], train_field, overfit=overfit), 'valid_dict': CDictionaryDataset(examples['valid'], valid_field, overfit=overfit), 'test_dict': CDictionaryDataset(examples['test'], valid_field, overfit=overfit)}
    collators = {'train': PairedCollator(train_field, device=device), 'valid': PairedCollator(valid_field, device=device), 'train_dict': DictionaryCollator(train_field, device=device), 'valid_dict': DictionaryCollator(valid_field, device=device), 'test_dict': DictionaryCollator(valid_field, device=device)}
    ...
    dataloaders = {}
    dataloaders['valid_dict'] = DataLoader(datasets['valid_dict'], batch_size=max(1, sc_batch_size * 2), num_workers=config.optimizer.num_workers, collate_fn=collators['valid_dict'])
    dataloaders['test_dict'] = DataLoader(datasets['test_dict'], batch_size=max(1, sc_batch_size * 2), num_workers=config.optimizer.num_workers, collate_fn=collators['test_dict'])
    if getattr(config.exp, 'eval', False):
        return dataloaders
    samplers = {'train': DistributedSampler(datasets['train'], shuffle=True), 'valid': DistributedSampler(datasets['valid'], shuffle=False), 'train_dict': DistributedSampler(datasets['train_dict'], shuffle=True)}
    ...
    dataloaders['train'] = DataLoader(datasets['train'], batch_sampler=batch_train_sampler, collate_fn=collators['train'], num_workers=config.optimizer.num_workers)
    dataloaders['valid'] = DataLoader(datasets['valid'], batch_size=batch_size, sampler=samplers['valid'], collate_fn=collators['valid'], num_workers=config.optimizer.num_workers)
    dataloaders['train_dict'] = DataLoader(datasets['train_dict'], batch_sampler=batch_train_dict_sampler, collate_fn=collators['train_dict'], num_workers=config.optimizer.num_workers)
    return (dataloaders, samplers)

def build_test_dataloaders(config=None, device='cpu', from_idx=0, to_idx=-1):
    datasets = {'test': TestDataset(root=os.path.join(os.environ['DATA_ROOT'], 'test2014'), anno_file=os.path.join(os.environ['DATA_ROOT'], 'annotations/image_info_test2014.json'), from_idx=from_idx, to_idx=to_idx), 'valid': TestDataset(root=os.path.join(os.environ['DATA_ROOT'], 'val2014'), anno_file=os.path.join(os.environ['DATA_ROOT'], 'annotations/captions_val2014.json'), from_idx=from_idx, to_idx=to_idx)}
    collator = TestCollator(device=device)
    sc_batch_size = config.optimizer.batch_size
    dataloaders = {}
    dataloaders['test'] = DataLoader(datasets['test'], batch_size=16, num_workers=4, collate_fn=collator)
    dataloaders['valid'] = DataLoader(datasets['valid'], batch_size=16, num_workers=4, collate_fn=collator)
    return dataloaders
